...
for r in range(Nr):
    ...
    if r==0:
            p = lmfit.Parameters()
            p.add_many(('nc', 0.8,True,0.8,1), ('Tc_pal', 10*10**5,True,1*10**5,10*10**5), ('Ts_pal', 17*10**5,True,1*10**5,20*10**5), ('Uc',0,True,-1.5,0),('Us',0,True,0,0.5))
    else:                          
            p = lmfit.Parameters()
            p.add_many(('nc', nc[r-1],True,0.8,1), ('Tc_pal', Tc_pal[r-1],True,1*10**5,10*10**5),  ('Ts_pal', Ts_pal[r-1],True,1*10**5,20*10**5), ('Uc',Uc[r-1],True,-1.5,-0.1),('Us',Us[r-1],True,0,17))

    f_11=np.zeros(shape = (Nv, 1))   #measured(calculated) VDF
    for i in range(Nv):
            f_11[i]=f_1[40*Nv+i,r]
    ...
    for i in range(Nv):
        solu2_c[i]=np.log10(fitting_c[i]/fitting_max)
    for i in range(Nv):
        solu2_s[i]=np.log10(fitting_s[i]/fitting_max)
    #for i in range(Nv):
